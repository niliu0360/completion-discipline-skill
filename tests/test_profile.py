...
class DistributionTests(unittest.TestCase):
    ...
    def test_native_completion_and_supersession(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            base = {
                "evidence_key": "ACC-001:automated_test",
                "producer": {"type": "AGENT", "name": "distribution-test"},
            ...
            first = {
                **base,
                ...
                "supports_claims": ["ACC-001 passed."],
                "supersedes": [],
            }
            first_path = project / "ev1.json"
            self.write_json(first_path, first)
            self.run_cmd(
                str(ROOT / "scripts" / "append_evidence.py"),
                "--record",
                str(record),
                "--entry",
                str(first_path),
            )
            complete = self.run_cmd(
                str(ROOT / "scripts" / "completion_check.py"),
                "--record",
                str(record),
                "--json",
                "--write-report",
    ...
    def test_legacy_example_migrates(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = self.run_cmd(
                str(ROOT / "scripts" / "migrate_legacy_workspace.py"),
                "--workdir",
                str(ROOT / "examples" / "legacy" / ".ai-work"),
                "--output",
                str(output),
            )
    ...
    def test_generated_source_boundary(self) -> None:
        ...
        self.assertTrue(
            (ROOT / "runtime" / "aicl" / "reconciliation.py").is_file()
        )
        self.assertTrue(
            (ROOT / "schemas" / "control-record.schema.json").is_file()
        )

    def test_generated_json_files_parse(self) -> None:
        ...
        for path in schemas:
            ...
            self.assertTrue(value.get("$id"), msg=str(path))
        native = json.loads(
            (
                ROOT
                / "examples"
        ...
        self.assertEqual("0.2.0", native["schema_version"])
        manifest = json.loads(
            (ROOT / "runtime" / "MANIFEST.json").read_text(encoding="utf-8")
        )
        self.assertTrue(manifest["generated"])
        self.assertEqual("completion-discipline", manifest["profile"])
