...
3. 代码或配置类 REQ 被标记为完成时，必须有明确验证证据；“测试通过了”这句话本身不算证据。
4. 延期不等于完成，必须写明原因和下一步。
5. 拒绝项只有在向用户明确披露后，才算被完整对账。
6. 未映射 REQ、未知引用、未完成任务或证据缺失都会阻止 `COMPLETE`。
7. 不得编造证据。只记录真实执行过的命令、产物、评审或观察结果。
